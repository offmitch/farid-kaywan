...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const resultsLabel = document.querySelector('.results-monthly-repayments span');
  ...
  form.addEventListener('submit', (e) => {
    ...
    const mortgageAmount = parseFloat(mortgageAmountInput.value.replace(/,/g, ''));
  ...
  function calculatePayment(principal, annualRatePercent, amortizationYears, frequency) {
    ...
    if (frequency === 'accelerated-bi-weekly' || frequency === 'accelerated-weekly') {
      const monthlyPayment = standardPayment(principal, annualRate, amortizationYears, 12);
    ...
    const payment = standardPayment(principal, annualRate, amortizationYears, periodsPerYear);
    return { payment, periodsPerYear };
  }

  function standardPayment(principal, annualRate, amortizationYears, periodsPerYear) {
